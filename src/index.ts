...
export const ledgerQueueServer = new WsFIFOServer(dataConnector).ready().then(() => {
    ...
    const ledgerQueueWorker = new LedgerQueueWorker(dataConnector, new WsFIFOClient());

    // LedgerApiHelper used by the API server for reading data and pushing operations to the queue
    const ledgerApiHelper = new LedgerApiHelper(dataConnector, new WsFIFOClient());
